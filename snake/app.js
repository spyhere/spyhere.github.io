let dots = '.'
let dotAnimation = setInterval(() => {
    document.querySelector('.loading span').innerHTML = dots;
    if (dots.length < 3) {
        dots += '.';
    }
    else {
...
let onPlayerStateChangeIndicator = true;
function onPlayerStateChange() {
    ...
    if (onPlayerStateChangeIndicator) {
        document.querySelector("#walking").play();
        onPlayerStateChangeIndicator = false;
    }
    if (key) {
        setTimeout(() => {bg.style.opacity = '100%'; gameBox.style.opacity = "100%"; key = false;
        interval = setInterval(game, mode); target = setTimeout(targetPosition, Math.round(Math.random()*mode*2));}, 4500);
//     changeBg = setTimeout(() => {var frame = document.getElementById("video");
//     frame.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');}, 4800)
    }
...
function game() {
        ...
        cells[snake[snake.length-1]].classList.contains("snake")) {
            ...
            if (!key.start) {
                let knocking = document.getElementById("knocking");
                knocking.load();
                knocking.play();
            }
